...
import { UserApiService } from 'src/app/api/userApi.service';
import { catchError, tap } from 'rxjs/operators';
import { FeedApiService } from 'src/app/api/feedApi.service';
...
export class AuthService {
  ...
  setTokenAndUser(token: string, user: User) {
    ...
    this.userApiService.setAuthToken(token);
    this.feedApiService.setAuthToken(token);
    this.currentUser$.next(user);
  }

  async login(email: string, password: string): Promise<any> {
    return this.userApiService.post('/users/auth/login',
  ...
  register(user: User, password: string): Promise<any> {
    return this.userApiService.post('/users/auth/',
